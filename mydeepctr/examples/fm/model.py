...
@LastEditTime: 2020-06-10 13:25:15
...
import collections
from models import model_dict, model_config_dict
...
def create_optimizer(loss, init_lr):
    ...
    for g, v in zip(grads, tvars):
        if g is not None:
            value_hist_summary = tf.summary.histogram(
                "{}/value/hist".format(v.name), v)
            value_summaries.append(value_hist_summary)
    ...
    for g, v in zip(grads, tvars):
        if g is not None:
            ...
            sparsity_summary = tf.summary.scalar(
                ...
                # 稀疏度检验， 计算0元素占的比例， relu中梯度为0的比例应该很大
    ...
    train_summary_op = tf.summary.merge([loss_summary, value_summaries])
    train_op = optimizer.apply_gradients(
        zip(grads, tvars), global_step=global_step)
    return (train_op, train_summary_op)


def udf_create_model(model_name, model_config, features, labels, embedding_initializer=None, mode=tf.estimator.ModeKeys.TRAIN):
    '''
    Parameters:
    -------------------
    ...
    model = Model(model_config=model_config, inputs=features, labels=labels, mode=mode)
...
def model_fn_builder(
        model_name, model_config, learning_rate=None,
    ...
    def model_fn(features, labels, mode, params):
        ...
        logits, total_loss = udf_create_model(
            model_name, model_config, features, labels, embedding_initializer, mode)
        # ==================================================================

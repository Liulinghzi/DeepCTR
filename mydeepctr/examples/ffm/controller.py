...
@LastEditTime: 2020-06-09 12:50:19
...
parser.add_argument("--field_list", type=str, default=None)
...
from models.ffm import FFMConfig
...
if args.field_list is None:
    ...
    field_list = [field.split(',') for field in args.field_list.split('/')]
    field_dict = {}
    for idx, field in enumerate(field_list):
        for feature in field:
            if feature in sparse_features:
                field_dict[feature] = idx
sparse_feature_columns = [SparseFeature(feature_name=feat, vocab_size=vocab_dict[feat], embedding_dim=3, field_id=field_dict[feat], num_fields=len(field_list)) for feat in sparse_features]
...
model_config = FFMConfig(dnn_feature_columns, linear_feature_columns, class_num=2)
